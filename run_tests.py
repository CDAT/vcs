...
class VCSTestRunner(cdat_info.TestRunnerBase):
    def _prep_nose_options(self):
        ...
        if self.args.vtk is not None:
            vtk_name = "vtk-cdat"
            cdat_info.run_command("conda install -f -y -c %s %s" % (self.args.vtk, vtk_name))
        return opt
...
workdir = os.getcwd()
runner = VCSTestRunner(test_suite_name, options=["--no-vtk-ui", "--vtk"], options_files=["tests/vcs_runtests.json"], get_sample_data=True)
