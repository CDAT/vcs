...
class TestVCSProjectionBasics(basevcstest.VCSBaseTest):
    def testProjectionBasics(self):
        ...
        for t in range(31):
            ...
            bad = []
            pos = []
            for param, val in vcs.VCS_validation_functions.proj_ok_parameters.items():
                if t in val[0]:
                    good.append(param)
                    pos.append(val[1])
                else:
            ...
            for att in bad:
                try:
                    ...
                    if success:
                        raise ValueError(
                            "Shouldn't have been able to set '%s' on projection of type %s" %
                            (att, b.type))
