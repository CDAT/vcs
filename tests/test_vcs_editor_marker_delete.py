...
class TestVCSEditor(basevcstest.VCSBaseTest):
    ...
    def testMarkerDelete(self):
        ...
        editor = c.target
        self.assertIsNotNone(editor, "Could not find an editable object at %i, %i" % (int(.1 * w), int(.1 * h)))
        print "Found an editable object"
        self.assertFalse(type(editor) != vcs.editors.marker.MarkerEditor, "Object found is not a marker")
        print "Found a marker object"
        self.assertEqual(editor.marker, m, "Did not find the correct marker, expected %s %s %s" % (m.name, "received", editor.marker.name))
        ...
        self.assertFalse(len(m.x) != len(m.y) != len(m.type) != len(m.color), "Did not delete all attributes on marker")
