...
class TestVCSAnimateIsoline(basevcstest.VCSBaseTest):
    def testVCSAnimateIsolineColored(self):
        ...
        ret = 0
        for p in pngs:
            ret += self.checkImage(p,os.path.join(self.basedir,'test_vcs_animate_isoline_colored',os.path.basename(p)),pngReady=True)
        if ret == 0:
