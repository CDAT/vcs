...
class TestVCSHatch(basevcstest.VCSBaseTest):
    def testHatchPattern(self):
        ...
        fill_info = self.x.createtext('fill_info')
        fill_info.angle = 45
        fill_info.height = 12
        ...
        # Create a title
        plot_title = self.x.createtext('plot_title')
        ...
        self.x.plot(plot_title, bg=self.bg)
        self.x.plot(fill_test, bg=self.bg)
        self.x.plot(fill_info, bg=self.bg)
        self.checkImage("test_vcs_hatches_patterns.png")
