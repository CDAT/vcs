...
class TestVCSTextsExtents(basevcstest.VCSBaseTest):
    def testTetExtents(self):
        ...
        # We have to specify the geometry to make sure that the size of the
        # canvas doesn't change between the init and the plot functions
        text = self.x.createtext()
        text.string = [
            "A very very very very long string",
            "A\nmult-line\nstring",
            "Short"]
        # Use any value for initial; then we'll manually "right align" using
        # the text extents
        text.x = [.1]
        text.y = [.1, .5, .9]
