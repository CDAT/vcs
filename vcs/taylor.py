...
class TDMarker(vcs.bestMatch):
    ...
    def _setstatus(self, value):
        self._status = VCS_validation_functions.checkElements(
            self,
            'status',
            value,
            VCS_validation_functions.checkOnOff)
    ...
    def _setline(self, value):
        self._line = VCS_validation_functions.checkElements(
            self,
            'line',
            value,
            self.checklineconnectingtype)
    ...
    def _setid(self, value):
        self._id = VCS_validation_functions.checkElements(
            self,
            'id',
            value,
    ...
    def _setid_color(self, value):
        self._id_color = VCS_validation_functions.checkElements(
            self,
            'id_color',
            value,
            VCS_validation_functions.checkColor)
    ...
    def _setsymbol(self, value):
        self._symbol = VCS_validation_functions.checkElements(
            self,
            'symbol',
            value,
            VCS_validation_functions.checkMarker)
    ...
    def _setcolor(self, value):
        self._color = VCS_validation_functions.checkElements(
            self,
            'color',
            value,
            VCS_validation_functions.checkColor)
    ...
    def _setline_color(self, value):
        self._line_color = VCS_validation_functions.checkElements(
            self,
            'line_color',
            value,
            VCS_validation_functions.checkColor)
    ...
    def _setline_type(self, value):
        self._line_type = VCS_validation_functions.checkElements(
            self,
            'line_type',
            value,
            VCS_validation_functions.checkLineType)
...
# self._referencecolor=src'black'
    ...
    def _setskillDrawLabels(self, value):
        value = VCS_validation_functions.checkYesNo(
            self,
            'skillDrawLabels',
            value)
        if value is not None:
...
# print 'referencecolor =',self.referencecolor
    ...
    def script(self, script_filename, mode='a'):
        ...
        if scr_type == '.scr':
            ...
            f.write(
                "   %s = v.gettaylordiagram('%s')\n" %
                (unique_name, self.name))
            f.write("else:\n")
            f.write(
                "   %s = v.createtaylordiagram('%s')\n" %
                (unique_name, self.name))

            f.write('%s.detail = %s\n' % (unique_name, repr(self.detail)))
            # maximum value of the standard deviaton, copied to the value of
            # the outter circle
            f.write('%s.max = %s\n' % (unique_name, repr(self.max)))
            ...
            f.write(
                '%s.skillDrawLabels = %s\n' %
                (unique_name, repr(
                    self.skillDrawLabels)))
            f.write(
                '%s.skillCoefficient = %s\n' %
                (unique_name, repr(
                    self.skillCoefficient)))
            f.write(
                '%s.referencevalue = %s\n' %
                (unique_name, repr(
                    self.referencevalue)))
            ...
            for i in range(self.Marker.number):
                ...
                f.write(
                    '    line_size = %s,\n' %
                    repr(
                        self.Marker.line_size[i]))
                f.write(
                    '    line_type = %s\n' %
                    repr(
                        self.Marker.line_type[i]))
                f.write(')\n')
            f.close()
        else:
            # Json type
...
# t.y=[numpy.ma.sin(numpy.ma.arccos(d1))*(d0+self._stdmax*self.Marker.yoffset[i]/100.)]
    ...
    def drawarrow(self, canvas, xloc, yloc, x1, y1, x2, y2, color):
        ...
        if x2 - x1 > 0:
            ...
            yx = yloc - self.arrowlength * \
                numpy.sin(Alpha) * self.outtervalue * \
                self.arrowbase * numpy.cos(alpha2)
            xx = xloc - self.arrowlength * \
                numpy.cos(Alpha) * self.outtervalue * \
                self.arrowbase * numpy.cos(alpha2)
        else:
            yx = yloc + self.arrowlength * \
                numpy.sin(Alpha) * self.outtervalue * \
...
# Cy.append(self.outtervalue*numpy.sin(self.quadrans/2.*numpy.pi))
    ...
    def drawFrame(self, canvas, data, wc):
        ...
        frame = createnewvcsobj(
            ...
            'line',
            'tdiag_f',
            self.template.line1.line)
        xtic1 = createnewvcsobj(
            canvas,
            'line',
            'tdgxt1',
            self.template.xtic1.line)
        xtic2 = createnewvcsobj(
            canvas,
            'line',
            'tdgxt2',
            self.template.xtic2.line)
        ytic1 = createnewvcsobj(
            canvas,
            'line',
            'tdgxt2',
            self.template.ytic1.line)
        ytic2 = createnewvcsobj(
            canvas,
            'line',
            'tdgyt2',
            self.template.ytic2.line)
        xmtic1 = createnewvcsobj(
            canvas,
            'line',
            'tdxt1',
            self.template.xmintic1.line)
        ymtic1 = createnewvcsobj(
            canvas,
            'line',
            'tdxt2',
            self.template.ymintic1.line)
        ymtic2 = createnewvcsobj(
            canvas,
            'line',
            'tdyt2',
            self.template.ymintic2.line)
        xmtic2 = createnewvcsobj(
            canvas,
            'line',
            'tdxt2',
            self.template.xmintic2.line)
        stdticks = createnewvcsobj(
            canvas,
            'text',
            'tic',
            self.template.xlabel1.texttable,
            self.template.xlabel1.textorientation)
        stdticks2 = createnewvcsobj(
            canvas,
            'text',
            'tc2',
...
# O.list()
        ...
        Oy = []
        Ox, Oy = self.getArc(
            self.referencevalue, val2=90. * self.quadrans, convert=False)
        Cx, Cy = self.getArc(
        ...
        if self.quadrans == 1:
            ...
            for v in levs.keys():
                if wc[0] < v < min(wc[1], wc[3]):
                    ...
                    ytic1y.append(
                        [self.convert(v, axis='y'), self.convert(v, axis='y')])
                    ytic1x.append(
                        [self.template.ytic1.x1, self.template.ytic1.x2])
            ytic1.x = ytic1x
            ytic1.y = ytic1y
            if ytic1.x != []:
        ...
        for v in levs.keys():
            if wc[0] < v < min(wc[1], wc[3]):
                ...
                xmtic1x.append(
                    [self.convert(v, axis='x'), self.convert(v, axis='x')])
                xmtic1y.append(
                    [self.template.xmintic1.y1, self.template.xmintic1.y2])
                xmtic2x.append(tx)
                xmtic2y.append(ty)
        xmtic2.x = xmtic2x
        xmtic2.y = xmtic2y
        ...
        for v in levs.keys():
            if wc[0] < v < min(wc[3], wc[1]):
                ...
                if v >= 0:
                    ...
                    # to 180 degree.
                    tx, ty = self.getArc(
                        abs(v), val1=val1 + 90, val2=val2 + 90)
                xtic2x.append(tx)
                xtic2y.append(ty)
                ticxs.append(V)
        ...
        for v in levs.keys():
            if 0. <= v <= 1. or (-1. <= v and self.quadrans == 2):
                ...
                tic = createnewvcsobj(
                    ...
                    'text',
                    'cor_',
                    self.template.ylabel2.texttable,
                    self.template.ylabel2.textorientation)
                tic.priority = self.template.ylabel2.priority
                dx = 1. + self.template.ylabel2.x - \
                    self.template.data.x2  # How far from data.x2 ?
                ...
                ytic2x.append([self.convert(x1 *
                                            min(wc[1], wc[3]) *
                                            dx1, axis='x'), self.convert(x1 *
                                                                         min(wc[1], wc[3]) *
                                                                         dx2, axis='x')])
                ytic2y.append([self.convert(y1 *
                                            min(wc[1], wc[3]) *
                                            dx1, axis='y'), self.convert(y1 *
                                                                         min(wc[1], wc[3]) *
                                                                         dx2, axis='y')])
        ytic2.x = ytic2x
        ytic2.y = ytic2y
        ...
        for v in levs.keys():
            if 0. <= v <= 1. or (-1. <= v and self.quadrans == 2):
                x1 = v
                y1 = numpy.sin(numpy.arccos(x1))
                ymtic2x.append([self.convert(x1 *
                                             min(wc[1], wc[3]) *
                                             dx1, axis='x'), self.convert(x1 *
                                                                          min(wc[1], wc[3]) *
                                                                          dx2, axis='x')])
                ymtic2y.append([self.convert(y1 *
                                             min(wc[1], wc[3]) *
                                             dx1, axis='y'), self.convert(y1 *
                                                                          min(wc[1], wc[3]) *
                                                                          dx2, axis='y')])
        ...
        tic = createnewvcsobj(
            ...
            'text',
            'cor__',
            self.template.yname.texttable,
            self.template.yname.textorientation)
        tic.priority = self.template.yname.priority
        # How far are we from yaxis ?
        ddx = 1. + self.template.yname.x - self.template.data.x1
    ...
    def rename(self, newname):
        ...
        if newname in vcs.elements["taylordiagram"].keys():
            raise Exception(
                ...
                newname)
        vcs.elements["taylordiagram"][newname] = vcs.elements[
