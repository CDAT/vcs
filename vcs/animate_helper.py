...
#            gm.legend = self.save_legend[i]
    ...
    def close(self, preserve_pngs=False):
        ...
        # Now that the animation is completed, restore the graphics methods min
        # and max values.
...
class AnimationCreate(StoppableThread):
    ...
    def run(self):
        ...
        for i, args in enumerate(all_args):
            ...
            if self._really_used == []:
                for d in displays:
                    self._really_used.append(
                        [d._gettemplate(), d._getg_type(), d._getg_name()])
...
class AnimationPlayback(StoppableThread):
    ...
    def run(self):
        ...
        while not self.is_stopped():
            self.wait_if_paused()
            # draw frame
            # print "DRAWING FRAME:", self.controller.frame_num,
            # self.controller.animation_files[self.frame_num]
    ...
    def create(self, thread_it=True, min=None, max=None):
        ...
        if self.create_thread is None or not self.create_thread.is_alive():
            ...
            if min is not None or max is not None:
                raise RuntimeError(
                    "Animation min/max autoset has been deprecated, use graphic method to set them")
