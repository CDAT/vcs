...
def update_input(canvas, dimensions, frame_num, update=True):
    # Ok let's loop through the arrays and figure out the slice needed and
    # update
...
class VTKAnimationCreate(animate_helper.StoppableThread):
    ...
    def get_frame(self, frame_num):
        ...
        self.controller.animation_files = sorted(
            glob.glob(
                os.path.join(
    ...
    def draw_frame(self, frame_num, png_name):
        ...
        update_input(
            self.canvas,
            self.controller._number_of_dims_used_for_plot,
            frame_num,
            update=False)
...
class VTKAnimate(animate_helper.AnimationController):
    ...
    def modified(self, obj, event):
        ...
        self.create_thread.canvas.backend.configureEvent(
            self.create_thread.canvas,
            "ModifiedEvent")

        if self.renderers is None or len(self.renderers) > 0:
            # If we're displaying a PNG, we'll skip the configureEvent action
            # on vcs_self's backend so it doesn't break anything
            self.vcs_self.backend._lastSize = new_size
            # All of the images are now the wrong size; need to blow them all
            # away.
            if self.animation_files:
                shutil.rmtree(os.path.dirname(self.animation_files[0]))
                self.animation_files = []
            # We'll use None as a sentinel value to tell us to replot in
            # retrieve_renderers
            self.renderers = None
    ...
    def extract_renderers(self):
        ...
        if self.modified_listener is None:
            self.modified_listener = self.vcs_self.backend.renWin.AddObserver(
                "ModifiedEvent",
                self.modified,
                25)
        ...
        # We don't want to render yet, because we are going to put a PNG on the
        # screen first.
    ...
    def draw_frame(self, frame_num=None, render_offscreen=False,
        ...
        if len(self.animation_files) == self.number_of_frames():
            ...
            # Retrieve the frame from the create thread and place it on the
            # canvas
            ...
            update_input(
                self.vcs_self,
                self._number_of_dims_used_for_plot,
                frame_num,
                update=False)
