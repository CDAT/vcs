...
def run(vcsinst, fname, baseline=None, threshold=defaultThreshold):
    """Export plot to a png and exit after comparsion."""
    vcsinst.png(fname)
    if baseline is None and len(sys.argv) > 1:
        baseline = sys.argv[1]
    sys.exit(check_result_image(fname, baseline, threshold))


def run_wo_terminate(vcsinst, fname, baseline=None,
...
def check_result_image(fname, baselinefname=None, threshold=defaultThreshold,
                       baseline=True, cleanup=True, update_baselines=False, suffix=""):
    testImage = image_from_file(fname)
    if baselinefname is None and len(sys.argv) > 1:
        baselinefname = sys.argv[1]
...
def main():
    if len(sys.argv) != 4:
        ...
        print "Call with ", sys.argv[0],
        print " test_directory/test_filename.png  baseline_directory/baseline_filename.png threshold"
        sys.exit(-1)
